...
estimateMvNormMeasurmentErrorEM <- function(X, se, max.iter=100, tol=1e-6) {
  # Initialize
  mu <- rowMeans(X)
  Sigma <- cov(t(X))
  ...
  for (iter in 1:max.iter) {
    theta.tm1 <- c(mu, Sigma)
    
    # Compute conditional mean and variance of true x's given observations
    m <- sapply(1:n, function(i)
      mu + Sigma %*% solve(Sigma + diag(se[, i]^2), X[, i] - mu))
    V <- Reduce(.Primitive('+'), lapply(1:n, function(i)
      Sigma - Sigma %*% solve(Sigma + diag(se[, i]^2), Sigma))) / n
    
    # Update estimates
    Sigma <- V + Reduce(.Primitive('+'), lapply(1:n, function(i)
      crossprod(t(m[, i, drop=FALSE])) )) / n
    mu <- rowMeans(m)
    theta <- c(mu, Sigma)
    
    # Check for convergence
    delta <- max(abs(theta - theta.tm1))
    if (delta < tol) {
      break
    }
  }
  return(list(mu=mu, Sigma=Sigma, iter=iter))
}
